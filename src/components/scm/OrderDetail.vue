      ...
      <dd class="content">
        <table class="col" id="orderinfo">
          ...
          <template v-else>
            <tr>
              <td colspan="4">
                주문내역이 비었습니다 DB 의 정합성이 맞지 않습니다. 팀원에게
                문의하세요
              </td>
            </tr>
          </template>
        ...
        <div id="dirArea">
          ...
          <div id="v-warehouse" v-if="shippingDirActivate">
                        ...
                        <tr>
                          <td>
                            <input
                              ...
                              :value="info.wh_id + ',' + info.sales_id"
            ...
            <template v-if="Object.keys(stocksMap).length == 0">
              <div class="my-5 text-center">
                <strong
                  >주문한 상품이 창고에 없습니다 발주를 먼저 진행하세요</strong
                >
              </div>
            </template>
          </div>
                ...
                <div class="salesinfo">
                  <template v-for="(ele, index) in item" :key="index">
                    <input
                      ...
                      v-if="index == 0"
                    />
                    <strong style="padding-right: 20px" v-if="index == 1">{{
                      ele
                    }}</strong>
                    <img
                      src="../../assets/images/treeview/minus.gif"
                      v-if="index == 1"
                      @click="cancel($event, ele)"
                    />
                  </template>
                </div>
          ...
          <div class="text-center mt-4">
            ...
            <button type="button" class="btn btn-light mx-2" @click="close">
              취소
            </button>
          </div>
        </div>
    ...
    writeDir: function () {
      ...
      if (this.directivesIsChecked(checkedDom)) {
        ...
        if (formType == "shippingDir") {
          ...
          if (
            this.shippingDirIsValidated(shippingTargets, targets, orderinfos)
          ) {
